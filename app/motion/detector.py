...
class MotionDetectorWorker:
    ...
    async def run(self) -> None:
        ...
        try:
            # Outer loop: acquire this camera's physical device slot,
            # watch frames until something higher-priority (live view,
            # audio, a snapshot) requests it, release, then try again --
            # see request_release()/release_requested() in
            # app/cameras/device_lock.py. This camera's hardware only
            # tolerates one RTSP connection at a time, shared with
            # recording and live view, so motion detection can't just
            # hold the stream open unconditionally the way it would on
            # hardware with no such limit.
            while self._running:
                await acquire_device_slot(url, timeout=None)
                cap = None
                try:
                    ...
                    last_sample = 0.0
                    while self._running:
                        if release_requested(url):
                            logger.debug("Motion detector: yielding device slot for camera %s", self.camera.id)
                            break
                        now = time.monotonic()
                    ...
                    if cap is not None:
                        cap.release()
                    release_device_slot(url)
                if not self._running:
                    return
                # Brief pause before trying to reacquire -- gives whatever
                # preempted us (typically live view) a moment to actually
                # take the connection instead of racing it for the slot.
