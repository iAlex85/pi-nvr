...
from app.cameras.device_lock import acquire_device_slot, release_device_slot, release_requested
...
class RecordingEngine:
    ...
    async def _start_camera(self, camera_id: int) -> None:
        ...
        state.yield_task = asyncio.create_task(self._yield_watcher(camera_id))
    ...
    async def _yield_watcher(self, camera_id: int) -> None:
        """Watches for a live-view/audio/snapshot preemption request on
        this camera's device (see request_release() in device_lock.py)
        and, if one comes in while we're actively holding the
        connection, terminates our process early so the slot frees up
        promptly instead of live view waiting out up to a full segment
        of recording. _watch_process's own exit-handling then takes care
        of releasing the slot and restarting us once it's free again --
        this is purely "notice the request and end the process sooner
        than it otherwise would have," not a second place that manages
        the connection itself."""
        while self._running:
            await asyncio.sleep(0.5)
            state = self._states.get(camera_id)
            if state is None or state.process is None or state.rtsp_url is None:
                return
            if state.process.returncode is not None:
                return  # already exited -- _watch_process handles the restart
            if release_requested(state.rtsp_url):
                logger.info("Recording for camera %s yielding device slot to a higher-priority request", camera_id)
                state.process.terminate()
                return
